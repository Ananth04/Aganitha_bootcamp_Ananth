...
class Chessboard {
  constructor() {
      ...
      //this.board = this.loadBoardFromFile('chessboard.yaml');
      this.placePieces();
  }
  ...
  loadBoardFromFile(filename) {
    try {
      ...
      const loadedBoard = yaml.load(yamlData);
      return loadedBoard;
    } catch (err) {
      console.error(`Error loading chessboard from ${filename}: ${err}`);
      return null;
    }
  }
}
...
const loadedchess = myChessboard.loadBoardFromFile('chessboard.yaml');
console.log(loadedchess);
